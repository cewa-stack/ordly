...
class Container:
    ...
    def returns_service(self, session: AsyncSession) -> ReturnsService:
        """Buduje ReturnsService dla GET /api/v1/returns."""
        return ReturnsService(SqliteReturnRepository(session))

    def issues_service(self, session: AsyncSession) -> IssuesService:
        """Buduje IssuesService dla /api/v1/issues (dyskusje i reklamacje)."""
        return IssuesService(self.build_plugin(session))

    def mail_service(self) -> MailService:
        """Buduje MailService dla /api/v1/mail (wysyłka do hurtowni)."""
        return MailService(self._settings.smtp)
    ...
    def shipping_reminder_service(self, session: AsyncSession) -> ShippingReminderService:
    ...
    def telegram_cleanup_service(self, session: AsyncSession) -> TelegramCleanupService:
