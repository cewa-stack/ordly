...
def register_mail_sync_job(
    ...
    """
    Rejestruje job synchronizacji skrzynki (IMAP) co N minut (domyślnie 5).

    Args:
        scheduler: Instancja schedulera zwrócona przez create_scheduler().
        job_coroutine: Bezargumentowa korutyna do wywołania cyklicznie.
        interval_minutes: Odstęp w minutach między wywołaniami.
    """
    scheduler.add_job(
        job_coroutine,
        trigger="interval",
    ...
    logger.info("Zarejestrowano job synchronizacji skrzynki co {} min", interval_minutes)
...
def register_shipping_reminder_job(
    ...
    """
    Rejestruje codzienny job przypomnienia o zamówieniach ze statusem NEW.
