...
class InventoryService:
    ...
    async def create_item(self, sku: str, name: str, min_stock: int = 0) -> InventoryItem:
        """
    ...
    async def link_offer(
        ...
        await self._repository.add_offer_link(marketplace, external_product_id, sku, quantity)
    ...
    async def unlink_offer(self, marketplace: str, external_product_id: str) -> int:
        ...
        return await self._repository.remove_offer_links(marketplace, external_product_id)
    ...
    async def get_report(self) -> InventoryReport:
        ...
        total_value = sum((item.stock_value for item in items), start=Decimal("0"))
