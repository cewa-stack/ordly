...
class AppSettings(BaseSettings):
    ...
    def validate_log_level(cls, value: str) -> str:
        ...
        if normalized not in allowed:
            raise ValueError(f"log_level musi być jednym z {allowed}, otrzymano: {value}")
        return normalized
...
class AllegroSettings(BaseSettings):
    ...
    def validate_environment(cls, value: str) -> str:
        ...
        if normalized not in allowed:
            raise ValueError(f"environment musi być jednym z {allowed}, otrzymano: {value}")
        return normalized
...
class BackupSettings(BaseSettings):
    ...
    backup_directory: Path = Field(default=Path("./backups"), alias="BACKUP_DIRECTORY")
...
class WebPushSettings(BaseSettings):
    ...
    def enabled(self) -> bool:
        ...
        return bool(self.vapid_public_key) and bool(self.vapid_private_key.get_secret_value())


class SmtpSettings(BaseSettings):
    """
    Konfiguracja SMTP do wysyłki maili do hurtowni (przycisk "Zamów u
    hurtowni" w aplikacji desktopowej).

    Sekrety żyją WYŁĄCZNIE tutaj, na Pi - aplikacja desktopowa nigdy nie
    ma hasła SMTP, tylko woła REST endpoint z gotową treścią maila.
    `enabled=False` (brak pełnej konfiguracji) oznacza czytelny błąd
    zamiast próby połączenia z pustym hostem.
    """
    ...
    password: SecretStr = Field(default=SecretStr(""), alias="SMTP_PASS")
    use_ssl: bool = Field(default=False, alias="SMTP_USE_SSL")

    @property
    def enabled(self) -> bool:
        """SMTP jest aktywny tylko, gdy host, użytkownik i hasło są ustawione."""
        return bool(self.host) and bool(self.user) and bool(self.password.get_secret_value())


class MailWatchSettings(BaseSettings):
    """
    Konfiguracja IMAP do wykrywania maili od marketplace (Allegro/OLX) -
    zakładka "Skrzynka" w aplikacji desktopowej.

    Puste wartości = obserwator wyłączony (job po prostu nic nie robi
    w danym cyklu, bez błędu). Dla Gmaila wymagane jest hasło aplikacji
    (2FA włączone) - zwykłe hasło konta Google nie zadziała, tak samo
    jak przy SMTP (patrz SmtpSettings).
    """
    ...
    host: str = Field(default="imap.gmail.com", alias="IMAP_HOST")
